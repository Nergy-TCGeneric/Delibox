...
def update_visualizing_image(index):
    ...
    while iteration_count < 9:
        another_sampled_data = g2_lidar.read_data()
        print(f"length: {len(scanned_data)}, {len(another_sampled_data)}")
        if len(scanned_data) < len(another_sampled_data):
            for i in range(len(scanned_data)):
                another_sampled_data[i].distance = (
                    another_sampled_data[i].distance * (iteration_count + 1)
                    + scanned_data[i].distance
                ) // (iteration_count + 2)
        else:
            for i in range(len(another_sampled_data)):
                scanned_data[i].distance = (
                    scanned_data[i].distance * (iteration_count + 1)
                    + another_sampled_data[i].distance
                ) // (iteration_count + 2)
        iteration_count = iteration_count + 1

    final = submapper.lidar_to_submap(scanned_data)
    global_mapper.update(final)
